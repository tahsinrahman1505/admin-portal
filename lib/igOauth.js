...
// Secure, Path=/, and has NO Domain attribute — which means a sibling subdomain
// cannot set a same-named Domain-scoped cookie and shadow it. Without the
// prefix, `request.cookies.get()` resolves such a duplicate by ambiguous
// precedence, which is the one remaining way to feed an attacker-chosen nonce
// into a victim's request. Costs nothing: this cookie already satisfies all
// three requirements. (Moot on *.vercel.app, which is on the Public Suffix
// List, but real the moment this serves from a custom tahsinai.com subdomain
// alongside siblings like demo.tahsinai.com.)
export const IG_NONCE_COOKIE = '__Host-ig_oauth_nonce'
...
export function verifyState(state) {
  ...
  // Exactly two segments. `split('.')` destructuring silently ignores anything
  // past the second, so `<valid-state>.junk` and `<valid-state>.x.y.z` all
  // verified as the same tenant — infinitely many strings mapping to one valid
  // state. Harmless today (the nonce is burned on every exit, so replay is
  // already dead), but it would quietly defeat any future replay tracking keyed
  // on the state string.
...
export function nonceMatches(cookieNonce, stateNonce) {
  // Type check before String() coercion: without it `nonceMatches({}, {})`
  // returns TRUE, because both sides coerce to '[object Object]'. Unreachable
  // from the current callers (a cookie value is always a string), but a
  // security primitive that returns "match" for two junk objects is a trap
...
export async function exchangeCodeForToken(code, redirectUri) {
  // requireSecret() here too, not just on the signing paths. Without it an
  // undefined secret is stringified into the request as the literal
  // "undefined" and we make a doomed remote call instead of failing with a
  // clear config error — which is precisely the "future caller reopens the
  // hole" case requireSecret exists to prevent.
  const secret = requireSecret()
...
export async function exchangeForLongLived(shortToken) {
  ...
  const url = `https://graph.instagram.com/access_token?grant_type=ig_exchange_token` +
    `&client_secret=${encodeURIComponent(secret)}&access_token=${encodeURIComponent(shortToken)}`
